...
def foward_propagation(X, parametres, tuple_size, dimensions):
    ...
    for c in range(1, C+1):
        ...
        Z = Z.reshape((tuple_size[c-1], tuple_size[c-1]))
        Z = np.clip(Z, -20, 20)

        if c < C:
            Z = reshape(Z, dimensions[str(c+1)][0] ,tuple_size[c-1])
...
def reshape(X, k_size_sqrt, x_size_sqrt):
...
dimensions = {}
dimensions = {"1": (2, "relu"), "2": (3, "relu"), "3": (5, "sigmoide")}
parametres, tuple_size = initialisation(X, dimensions, 0, 1)
...
K = parametres["K1"]
X = reshape(X, dimensions["1"][0], x_shape)
