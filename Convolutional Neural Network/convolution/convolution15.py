...
for i in range(dZ.shape[0]+k_size_sqrt-1):
    for j in range(dZ.shape[1]+k_size_sqrt-1): 
        print(np.convolve(new_dZ[i:i+k_size_sqrt, j:j+k_size_sqrt].flatten(), K, "valid"))
"""
...
def error_initialisation(x_shape0, list_size, dimension, input_size, previ_input_size, type_layer, fonction, stride):

    if input_size < 1:
        show_information(x_shape0, list_size, dimension)
...
def initialisation_extraction(dimension, i):
    ...
    type_layer = dimension[str(i)][3]
    fonction = dimension[str(i)][4]

    return k_size, stride, padding, type_layer, fonction
...
def initialisation_calcul(x_shape0, dimension, padding_mode):
    ...
    for i in range(1, len(dimension)+1):
        ...
        k_size, stride, padding, type_layer, fonction = initialisation_extraction(dimension, i)
        ...
        if input_size % stride != 0 and padding_mode == "auto":
            ...
            dimension[str(i)] = k_size, stride, padding, type_layer, fonction
        ...
        error_initialisation(x_shape0, list_size, dimension, input_size, previ_input_size, type_layer, fonction, stride)
...
def initialisation_affectation(dimension, list_size):
    ...
    parametres_grad = {}
    for i in range(1, len(dimension)+1):
        k_size, _, _, type_layer, fonction = initialisation_extraction(dimension, i)
        o_size = list_size[i-1]

        if type_layer == "kernel":
            parametres, parametres_grad = initialisation_kernel(parametres, parametres_grad, k_size, o_size, type_layer, fonction, i)
...
def initialisation(x_shape0, dimension, padding_mode):

    dimension, list_size = initialisation_calcul(x_shape0, dimension, padding_mode)
...
def add_padding(X, padding):
    ...
    h_padding = np.zeros((X.shape[0], padding))
    v_padding = np.zeros((padding, X.shape[1] + padding))    
...
def main():
    ...
    dimensions = {}
    dimensions = {"1" :(3, 1, 0, "kernel", "relu"),
                  "2" :(2, 2, 0, "pooling", "max"),
                  "3" :(3, 1, 0, "kernel", "relu"),
                  "4" :(2, 2, 0, "pooling", "max"),
                  "5" :(5, 1, 0, "kernel", "sigmoide")}

    x_shape0 = X.shape[0]
    padding_mode = "auto"
    parametres, parametres_grad, dimensions, tuple_size = initialisation(x_shape0, dimensions, padding_mode)

    show_information(x_shape0, tuple_size, dimensions)
