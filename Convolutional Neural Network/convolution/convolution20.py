...
def correlate(A, K, b, x_size):
    ...
    in_channels = A.shape[0]
    out_channels = K.shape[0]
    nb_kernels_per_channel = out_channels // in_channels

    layers = []

    for in_ch in range(in_channels):
        for k in range(nb_kernels_per_channel):
            out_ch = k + in_ch * nb_kernels_per_channel
            Z = A[in_ch] @ K[out_ch] + b[out_ch]
            Z = Z.reshape((x_size, x_size))
            layers.append(Z)

    Z_concat = np.stack(layers, axis=0)
...
def initialisation_kernel(parametres, parametres_grad, k_size, o_size, nb_kernel, type_layer, fonction, i):
    ...
    parametres["K" + str(i)] = np.random.rand(nb_kernel, k_size**2, 1).astype(np.float16) * 2 - 1
    parametres["b" + str(i)] = np.random.rand(nb_kernel, np.int64(o_size)**2, 1).astype(np.float16) * 2 - 1 #np.int64 avoid overflow with o_size**2
    parametres["l" + str(i)] = type_layer
    parametres["f" + str(i)] = fonction

    parametres_grad["m" + str(i)] = np.zeros((nb_kernel, k_size**2, 1)).astype(np.float16)
    parametres_grad["v" + str(i)] = np.zeros((nb_kernel, k_size**2, 1)).astype(np.float16)
