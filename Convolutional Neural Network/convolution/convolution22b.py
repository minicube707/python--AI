...
max_pooling :
=========DESCRIPTION=========
Performs max pooling over each channel of the input tensor X.

=========INPUT=========
X (np.ndarray): Activation tensor of shape (channels, height, width)
kernel_size (int): Size of the pooling kernel (assumed square)
...
def max_pooling(X, kernel_size, stride):
    ...
    out_height = output_shape(height, kernel_size, stride)
    out_width = output_shape(width, kernel_size, stride)

    Z = np.zeros((channels, out_height, out_width)).astype(np.float32)
    ...
    for c in range(channels):
        for i in range(out_height):
            for j in range(out_width):
                ...
                Z[c, i, j] = np.max(X[c, h_start:h_end, w_start:w_end])
...
def correlate(A, K, b, stride, padding):
    ...
    in_channels, H, W = A.shape
    out_channels, kH, kW = K.shape

    H_out = output_shape(H, kH, stride, padding)
    W_out = output_shape(W, kW, stride, padding)

    output = np.zeros((out_channels, H_out, W_out)).astype(np.float32)
    nb_kernel_by_acti = out_channels // in_channels

    for ic in range(in_channels):
        for oc in range(nb_kernel_by_acti):
            for i in range(H_out):
                for j in range(W_out):
                    ...
                    patch = A[ic, h_start:h_start + kH, w_start:w_start + kW].astype(np.float32)
                    output[oc + ic * nb_kernel_by_acti, i, j] = np.sum(patch * K[oc])
...
tuple           next_shape :    tuple containe the shape for the next derivative

=========OUTPUT=========
numpy.array    next_dZ :       Array containe the derivated for the next layer
"""
@njit
def convolution(dZ, K, next_shape):

    k_size = K.shape[1]

    #next_dz is the output
    next_dZ = np.zeros(next_shape).astype(np.float32)
    ...
    for c in range(C):
        ...
        kernel = np.rot90(K[c], 2)  # Proper 180° rotation
        for i in range(H):
            for j in range(W):
                region = dZ[c, i:i + k_size, j:j + k_size]
                next_dZ[c, i, j] = np.sum(region * kernel)
...
"""
@njit
def output_shape(input_size, k_size, stride, padding = 0):
...
def initialisation_kernel(parametres, parametres_grad, k_size, o_size, nb_kernel, type_layer, fonction, i):
    ...
    parametres["K" + str(i)] = np.random.rand(nb_kernel, k_size, k_size).astype(np.float32) * 2 - 1
    parametres["b" + str(i)] = np.random.rand(nb_kernel, o_size, o_size).astype(np.float32) * 2 - 1
    parametres["l" + str(i)] = type_layer
    parametres["f" + str(i)] = fonction

    parametres_grad["m" + str(i)] = np.zeros((nb_kernel, k_size, k_size)).astype(np.float32)
    parametres_grad["v" + str(i)] = np.zeros((nb_kernel, k_size, k_size)).astype(np.float32)
...
def back_propagation_kernel(activation, parametres, dimensions, gradients, dZ, c):
    ...
    dK = np.zeros(parametres["K" + str(c)].shape).astype(np.float32)
    ...
    if c > 1:
        ...
        k_size = parametres["K" + str(c)].shape[1]
        padded_dZ = np.pad(dZ, pad_width=((0, 0), (k_size - 1, k_size - 1), (k_size - 1, k_size - 1)), mode='constant', constant_values=0)
        dZ = convolution(padded_dZ, parametres["K" + str(c)], activation["A" + str(c-1)].shape)
...
def add_padding(X, padding):
    return np.pad(X, pad_width=((0, 0), (0, padding), (0, padding)), mode='constant', constant_values=0).astype(np.float32)
...
def main():
    ...
    x_shape = 21
    X = np.random.rand(x_shape, x_shape).astype(np.float32)
    ...
    y_shape = o_size
    y = np.random.rand(tuple_size_activation[-1][0], y_shape, y_shape).astype(np.float32)
