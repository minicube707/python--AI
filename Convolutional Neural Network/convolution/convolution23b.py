...
"""
==========DESCRIPTION=========
 Apply max pooling on activation map A.

 
==========INPUT=========
A (np.ndarray)      : Activation matrix (shape: [channels, height, width])
kernel_size (int)   : Size of the pooling window (assumed square)
stride (int)        : Stride of the pooling operation

==========OUTPUT=========
numpy.array x       : Array containing the pooled activation map
"""
def max_pooling(A, kernel_size, stride):

    C, H, W = A.shape
    out_h = output_shape(H, kernel_size, stride, 0)
    out_w = output_shape(W, kernel_size, stride, 0)

    Z = np.zeros((C, out_h, out_w))
    ...
    for c in range(C):
        for i in range(out_h):
            for j in range(out_w):
                ...
                Z[c, i, j] = np.max(A[c, h_start:h_end, w_start:w_end])
...
K (np.ndarray)      : Kernel matrix (shape: [nb kernel, layer, height, width])
...
def correlate(A, K, b, stride, padding):
    ...
    C_in, H_in, W_in = A.shape
    N_k, _, H_k, W_k = K.shape

    # Compute output size
    H_out = output_shape(H_in, H_k, stride, padding)
    W_out = output_shape(W_in, W_k, stride, padding)

    # Output activation map
    Z = np.zeros((N_k, H_out, W_out))

    # Correlation operation
    for n in range(N_k):           # over kernels
        for i in range(H_out):
            for j in range(W_out):
                h_start = i * stride
                w_start = j * stride
                region = A[:, h_start:h_start + H_k, w_start:w_start + W_k]
                Z[n, i, j] = np.sum(region * K[n])
...
"""
def convolution(dZ, K, k_size):
     
    L_dZ, H_dZ, W_dZ =  dZ.shape
    NB_K, L_K, H_K, W_K = K.shape

    #new_dz is intput with a pas to do the the cross product with all value
    new_dZ = np.pad(dZ, pad_width=((0, 0), (k_size - 1, k_size - 1), (k_size - 1, k_size - 1)), mode='constant', constant_values=0)

    #next_dz is the output
    next_dZ = np.zeros((L_K, H_dZ+k_size-1, W_dZ+k_size-1))
        ...
        for b in range(next_dZ.shape[0]):
            ...
            for c in range(next_dZ.shape[1]):
                ...
                for d in range(next_dZ.shape[2]): 
                    ...
                    #DO THE CONVOLUTION
                    next_dZ[b, c, d] += np.dot(dZ_layer[b, c:c + k_size, d:d + k_size].flatten(), tensor_K[b][::-1].flatten())
...
def initialisation_kernel(parametres, parametres_grad, list_size_activation, k_size, type_layer, fonction, i):
    ...
    parametres["K" + str(i)] = np.random.rand(nb_kernel, nb_layer, k_size, k_size).astype(np.float16) * 2 -1
    parametres["b" + str(i)] = np.random.rand(nb_kernel, o_size, o_size).astype(np.float16) * 2 - 1
    parametres["l" + str(i)] = type_layer
    parametres["f" + str(i)] = fonction

    parametres_grad["m" + str(i)] = np.zeros((nb_kernel, nb_layer, k_size, k_size)).astype(np.float16)
    parametres_grad["v" + str(i)] = np.zeros((nb_kernel, nb_layer, k_size, k_size)).astype(np.float16)
...
"""
def pooling_activation(A, kernel_size, stride):
    Z = max_pooling(A, kernel_size, stride)
    return Z
...
numpy.array     Z   : the resultat of the activation matrice after pass throw the activation function
"""
def kernel_activation(A, K, b, mode, stride, padding):
...
numpy.array     Z   : the resultat of the activation matrice after pass throw the activation function
"""
def function_activation(A, K, b, mode, type_layer, x_size, stride, padding):

    #Activation are in line format
    if type_layer == "kernel":
        Z = kernel_activation(A, K, b, mode, stride, padding)
    else:
...
def foward_propagation(X, parametres, tuple_size_activation, dimensions):
    ...
    for c in range(1, C+1):
        ...
        activation["A" + str(c)] = function_activation(A, K, b, mode, type_layer, x_size, stride, padding)
...
def back_propagation_pooling(activation, parametres, dimensions, dZ, c):
    ...
    stride = dimensions[str(c)][1]
    k_size = parametres["K" + str(c - 1)].shape[2]  # suppose carré : kH == kW
...
def back_propagation_kernel(activation, parametres, dimensions, gradients, dZ, c):
    ...
    A_prev = activation["A" + str(c - 1)]  # shape: (C_in, H_in, W_in)
    K = parametres["K" + str(c)]           # shape: (C_out, C_in, H_k, W_k)
    stride = dimensions[str(c)][1]
    padding = dimensions[str(c)][2]

    C_out, C_in, H_k, W_k = K.shape
    _, H_in, W_in = A_prev.shape
    _, H_out, W_out = dZ.shape

    # Apply padding to input
    A_prev_padded = np.pad(A_prev, ((0, 0), (padding, padding), (padding, padding)), mode='constant')

    dK = np.zeros_like(K)

    # Loop to compute gradient of kernel
    for i in range(C_out):          # over filters
        for j in range(C_in):       # over input channels
            for h in range(H_out):
                for w in range(W_out):
                    h_start = h * stride
                    w_start = w * stride
                    region = A_prev_padded[j, h_start:h_start + H_k, w_start:w_start + W_k]
                    dK[i, j] += region * dZ[i, h, w]
