...
def initialisation_kernel(parametres, parametres_grad, list_size_activation, k_size, type_layer, fonction, i):
    ...
    parametres["b" + str(i)] = np.zeros((nb_kernel, np.int64(o_size)**2, 1)).astype(np.float16) #np.int64 avoid overflow with o_size**2
    parametres["l" + str(i)] = type_layer
    parametres["f" + str(i)] = fonction
...
def display_activation(X, y, parametres_CNN, dimensions_CNN, alpha, y_shape):
    ...
    # Afficher l'image X
    plt.subplot(1, 2, 1)
    img = deshape(X, dimensions_CNN["1"][0], dimensions_CNN["1"][1])
    ...
    tuple_size_activation = create_tuple_size((3, 28, 28), dimensions_CNN)
    ...
    for i in range(1, len(dimensions_CNN)):
        new_A = deshape(activations_CNN["A" + str(i)], dimensions_CNN[str(i+1)][0],  dimensions_CNN[str(i+1)][1])
        new_Z = activations_CNN["Z" + str(i)]
