...
numpy.array     x :     array containe the next activation
"""
def max_pooling(X, K, stride):
    new_X = np.zeros((X.shape[0], X.shape[1] // stride, X.shape[2] // stride))
    for i in range(X.shape[0]):
        for j in range(0, X.shape[1], stride):
            for k in range(0, X.shape[2], stride):
                new_X[i, j // stride, k // stride] = np.max(X[i, j:j+K, k:k+K])
...
def initialisation_kernel(parametres, parametres_grad, list_size_activation, k_size, type_layer, fonction, i):
    ...
    K = np.zeros(shape).astype(np.float32)
    center = k_size // 2
    K[:, :, center, center] = 1.0
