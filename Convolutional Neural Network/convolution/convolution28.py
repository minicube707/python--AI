...
"""
@njit
def forward_propagation(X, parametres_K, parametres_B, 
                        activations_A, activations_Z, tuple_mode_info, alpha):

    C = len(tuple_mode_info)

    activations_A[0] = X.astype(np.float32)  # A0
    activations_Z[0] = X.astype(np.float32)  # Z0 Useless, but usefull to keep align with A
    ...
    for c in range(C):
        ...
        activations_A[c+1] = A_next
        activations_Z[c+1] = Z_next
...
def update(
    ...
    for c in range(C):
        ...
        if tuple_mode_info[c][0] == 0:  # kernel layer
            ...
            # update moments
            m_list[c] = beta1 * m_list[c] + (one - beta1) * gradients_dK[c]
            v_list[c] = beta2 * v_list[c] + (one - beta2) * (gradients_dK[c] ** 2)

            # biais correction
            m_hat = m_list[c] / (one - np.float32(beta1**(c+1)))
            v_hat = v_list[c] / (one - np.float32(beta2**(c+1)))
...
def display_activation(X, y, parametres_K, parametres_B, tuple_mode_info, alpha):
    ...
    array_3dtype = types.float32[:, :, :]
    activations_A = List.empty_list(array_3dtype)
    activations_Z = List.empty_list(array_3dtype)
    for _ in range(C_CNN+1):
        activations_A.append(np.zeros((1,1,1), dtype=np.float32))
        activations_Z.append(np.zeros((1,1,1), dtype=np.float32))

    activations_A, activations_Z = forward_propagation(X, 
        parametres_K, parametres_B, 
        activations_A, activations_Z,
        tuple_mode_info, alpha)
...
def train_loop(X, y, parametres_K, parametres_B,
    ...
    for it in range(nb_iteration):
        ...
        """
        print("")
        print(forward_propagation.signatures)
        print(back_propagation_CNN.signatures)
        print(update.signatures)
        print(log_loss.signatures)
        print(accuracy_score.signatures)
        print(dx_log_loss.signatures)"""
...
def main():
    ...
    display_activation(X, y, parametres_K, parametres_B, tuple_mode_info, alpha)
